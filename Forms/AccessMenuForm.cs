    ...
    public partial class AccessMenuForm : Form
    {
        ...
        public AccessMenuForm(LoginForm parent, IConfiguration config)
        {
            ...
            this.FormClosing += AccessMenuForm_FormClosing;
                    // Adicionar handler para o botão Parâmetros
                    this.btnParametros.Click += btnParametros_Click;
        }
        ...
        private void btnParametros_Click(object sender, EventArgs e)
        {
            try
            {
                // Abrir tela de parâmetros
                var parametrosForm = new ParametrosForm(_config);
                parametrosForm.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show(
                    $"Erro ao abrir tela de parâmetros:\n{ex.Message}",
                    "Erro",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error
                );
